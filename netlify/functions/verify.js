exports.handler = async function(event) {
  ...
  try {
    ...
    if (!data.content || !Array.isArray(data.content)) {
      throw new Error('Неожиданный ответ от API: ' + JSON.stringify(data));
    }
    ...
    try {
      ...
      throw new Error('Не удалось разобрать ответ ИИ: ' + raw);
    }
    ...
    return {
      statusCode: 200,
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ ai_steps: null, note: 'Ошибка: ' + err.message, status: 'На проверке' })
    };
  }
};
